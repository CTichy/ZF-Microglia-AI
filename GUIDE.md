...
2. **Protects skin as its own label**, before any real cell is touched — the same engine as **Protect Skin as Label** (below), using the brain mask layer only to *seed* skin (never to clamp its result — see that section's own note on this), at the same `lo` real cells get in step 4 (no separate stricter value — that margin existed only to keep skin from grabbing marginal real-cell-adjacent signal back when its own trim just excluded a touching cell outright; now that it jointly resolves the boundary against one instead, the margin isn't needed). Skin becomes a real, ordinary label (ID **-1**) at this point, with the same auto-grow/until-stable/joint-adjacent-resolution behavior Protect Skin as Label itself has — it adapts to whatever the cells currently look like (still raw, not yet auto-corrected) without ever modifying them. Once step 4 gets to each cell's own turn, that cell is free to adjust both itself and skin.
...
4. **Resorts every real cell by Centroid Z** — the same engine as **Resort Labels** (below) — giving step 5's wave grouping a stable, deterministic starting order.
5. **Corrects every cell at the calibrated `lo`**, batched into parallel-safe **waves** rather than strictly one at a time — the same auto-grow + until-stable engine as **Correct Label**'s **3D (whole cell, from centroid)** mode (below), fixed for this pipeline at: base pad 15px, auto-grow up to 10 attempts/slice growing 5px at a time, until-stable up to 100 passes/slice. Two cells whose own maximum-possible working areas can never overlap are provably unable to interact, so they're corrected concurrently (up to 75% of CPU cores); cells that *can* conflict are pushed into a later wave, which still sees every earlier wave's own already-corrected state — the same "resolve against an already-corrected neighbor" behavior a strictly one-at-a-time loop would give, just faster. Confirmed on real, densely-packed fish (one with literally zero cells fully isolated from every other) that this still yields a real speedup, and confirmed the batched result is byte-identical to what the old one-at-a-time loop would have produced. Each cell's own per-slice walk already resolves a genuinely adjacent label (skin, or another already-corrected cell) entirely on its own — no separate joint-watershed pass is needed after this loop.
...
**Bbox padding (px)** — the STARTING pad for each slice; with auto-grow on (the default), a slice that needs more room grows past this on its own, so this mainly sets how far the first attempt reaches before growth kicks in.
