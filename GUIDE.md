...
1. **Calibrates the contrast threshold** that best reproduces the labels the run just produced, straight from the raw signal — the exact same self-referential engine as [Calibrate Correct-Label Contrast (Section 9i)](#9i-calibrate-correct-label-contrast-from-cellpose-sam), just run against this run's own output instead of a hand-picked sample set. Call this calibrated value `lo`. **Only microglia count** in this step: any skin label already present is treated as plain background for the sweep, so the calibrated `lo` can never depend on whether skin happened to be protected first.
...
Saves the auto-corrected result to `<stem>_cp_krendl_ac.tif`, then — if sanding is enabled — saves the sanded result on top as `<stem>_cp_krendl_ac_snd.tif`, updating the `*_cellpose_labels` layer in place at each stage; `<stem>_cp.tif`/`<stem>_cp_krendl.tif` are unaffected — those stay the raw Krendl-pipeline output, unchanged. On-disk naming is cumulative: each stage's filename is the previous one with one more suffix appended (see [9f's naming table](#9f-build-gt-correction-package)), so the filename alone tells you which stages actually ran. The status line reports the calibrated `lo`, the skin label ID, how many cells were corrected vs. skipped (in Centroid-Z order), and how much debris was removed; the log box gets one **consolidated report covering every cell** — skin protection summary, then each corrected cell's own detail (group/touching labels, which slices needed a bigger pad, which took more than one pass to settle, converged or not, debris removed) in the same format Correct Label's own 3D report uses, plus an overall tally at the end.
...
**Label to correct** — type it directly, or click the label in the viewer and use **Use selected**. Accepts any positive label ID, or **-1** for the [Protect Skin as Label](#protect-skin-as-label) sentinel itself — useful for re-trimming skin's own territory by hand after it's been created. 0 and any other negative value are rejected outright (there's never a real label there).
...
**Signal layer** — the Image layer the contrast sweep and the skin trim run on (the skin trim automatically uses that layer's raw imported channel when it is open, since `_ExtRm`/`_NoBG` layers are zeroed outside the brain). Its contrast **low** limit is set to the calibrated `lo` afterwards.
...
**Hide skin label in the viewer** — a checkbox right below Remove Skin Label, using the same field. Purely visual: makes that label ID fully transparent in the active Labels layer's own display, without touching any data at all — unlike Remove Skin Label, nothing is deleted. Every OTHER label keeps its own real, distinct color exactly, both while hidden and after toggling back off (skin's own color returns too). Useful whenever skin's own territory is visually cluttering the view of the real cells you're correcting.
...
Runs the exact same 6-step pipeline the Cellpose-SAM Segmentation section's own [Auto-correct labels via contrast sweep](#auto-correct-labels-via-contrast-sweep-after-segmentation-checkbox-on-by-default) checkbox chains onto a fresh run — but against the **active Labels layer as it currently stands** instead of a just-produced one. This means a labels layer segmented earlier — either route, an earlier session, or one loaded via [Load Labels layer (.tif)](#load-labels-layer-tif) — can get the same treatment (self-calibrated contrast sweep, skin protection + debris cleanup, per-cell correction — 2D against skin for a touching cell, 3D otherwise — then a final debris cleanup) without re-running Cellpose-SAM segmentation from scratch.

Uses the same **Signal layer** / **Brain mask layer** / **Bbox padding (px)** fields as Protect Skin as Label, just above — this pipeline's own skin-protection step needs exactly the same two layers that button does, so there's no separate, duplicate set of fields to fill in.

Click **Auto-correct Labels**. On success, the active Labels layer is updated in place (skin protected as its own label, every real cell corrected), the signal layer's own contrast lower limit moves to the calibrated threshold (upper limit left untouched), Protect Skin as Label's own section is populated as if that button had been clicked directly, and the full consolidated report (same format the chained Cellpose-SAM pipeline produces, including the *possible non-microglia blobs* list) appears in the log box below the status line.
...
Typical times on one real fish (33 cells, 101×2048×2048): **Protect Skin ≈ 7 min** (about 6.5 min is the per-slice skin trim), **Auto-correct ≈ 4 min**. Crowded fish with 70–90 cells take longer. Watch the **status line**: it names each step, and the last steps are *"removing debris…"*, *"checking for possible non-microglia blobs…"*, *"Computation finished — applying the corrected labels to the viewer…"* and *"Applying the calibrated contrast to the signal layer…"* — so you can tell whether the computation or the hand-over to napari is slow. The terminal napari was started from prints how long the hand-over took, and — if a run is still going after 10 minutes — every thread's stack, repeated every 10 minutes. If the same lines repeat, send that output.
...
| Protect Skin as Label | Brain mask layer | Fills everything outside the brain mask with a real, protected label (ID -1 by default), trims it to real signal per-slice (parallel, no brain-mask clamp), then sweeps stray debris it absorbed — so no other label can ever bleed into skin residue again. Reports which real labels end up touching/nearby skin, worth a closer look. First calibrates `lo` with the contrast sweep on your labels (never the layer's old low limit), sets only the signal layer's low limit to it, and refuses to run if skin is already protected |
| Auto-correct Existing Labels | Signal + Brain mask layer (Protect Skin as Label's own fields) | Runs the full Cellpose-SAM auto-correct pipeline (contrast sweep, skin protection, per-cell 2D-vs-skin/3D correction, debris cleanup) against the active Labels layer as it stands — for a labels layer segmented earlier, any route or session. Reuses skin if already protected, refuses to correct cells without skin, decides "touching skin" by geometry (≤ 3 px), and lists possible non-microglia blobs (report only) |
