...
**Min volume is measured from the GT, not read from the slider — and it's a floor that only ever decreases.** The small-blob cleanup threshold used during the sweep is the true smallest labeled cell's own voxel volume in the GT labels you provided, not whatever the Min volume slider above happens to show. A fixed guessed number (this used to default to a flat 7500 regardless of fish) risks discarding a real small cell as noise if it's too high. But the fix isn't to just overwrite the slider with each fish's own measurement either: once one fish's GT proves a cell of a given size is real, a *different* fish's sweep — which may simply lack any cell that small — shouldn't be allowed to raise the threshold back above it. So applying a sweep's result takes the **smaller** of what this sweep just measured and whatever's currently in the Min volume slider, never the larger — the field only ratchets down as more GT evidence accumulates across sweeps, never up. The measured value (and whether it lowered the floor) is reported in the sweep's status line and **applied to the Min volume slider itself** once the sweep finishes (see below), so it becomes the default for a normal Create Labels run afterward too.
