...
**Hide skin label in the viewer** — a checkbox right below Remove Skin Label, using the same field. Purely visual: makes that label ID fully transparent in the active Labels layer's own display, without touching any data at all — unlike Remove Skin Label, nothing is deleted, so toggling the checkbox back off restores the layer's exact original colormap, real per-cell colors included. Useful whenever skin's own territory is visually cluttering the view of the real cells you're correcting.

  **While hidden, every OTHER label shows as one flat gray instead of its own distinct color — deliberate, not a bug.** An earlier version instead scanned the whole label volume and looked up every real cell's own individual color to preserve it while skin was hidden — for a large fish (hundreds of labels across a full-resolution stack) that scan alone measured around 1.5 seconds, running synchronously and freezing the interface on every single toggle. Falling back to one shared placeholder color for everything except skin needs neither step, so toggling stays fast (well under a second even the very first time in a session, near-instant afterward) regardless of fish size or label count. Toggling back off instantly restores every cell's real, original color exactly, since the layer's true colormap is only ever swapped out, never discarded.
