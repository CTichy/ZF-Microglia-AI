...
- **3D (whole cell, from centroid)** — corrects the entire cell. In-plane (Y/X), it's the same single padded window as 2D mode, applied per slice: every pixel at or above the threshold within the label's own bbox + padding counts, connected or not (see the note above). In depth (Z), the label's own original Z range is filled outright — it already exists on every one of those slices — then **extended one slice at a time** past each end: before accepting a new slice, the tool checks whether that slice's own signal actually touches (a 1-pixel dilation of) the *immediately preceding* slice's own accepted shape; the moment a step finds no such continuation, growth in that direction stops and nothing is written to that slice — everything already accepted stands. A hard cap (the same value as **Bbox padding** unless auto-grow overrides it) bounds how far this can ever extend beyond the label's own original range regardless, so a real, different, genuinely-touching structure can't hijack the growth indefinitely just because it happens to overlap slice-to-slice.

  (Two earlier versions were tried and replaced: the first walked outward testing only "is there *any* signal left in the padded window on the next slice" with no cap — on real, dense data that's far too weak a stop condition, since unrelated signal anywhere in that wide window kept it going, in the worst case through most of the fish's remaining depth; the second filled one big fixed box the full padding distance in Z regardless of whether the cell actually reached that far, which was fast and safe but wasteful and imprecise. Testing continuation against the immediately preceding slice's own shape finds the TRUE Z extent on its own in the normal case, while the cap still catches the genuinely-touching-neighbor case the fixed box was built to guard against.)

  Because growth only ever adds a slice after confirming real continuation, and the original range is always kept in full, there's no separate "trim leftover pixels beyond where growth stopped" step any more either.
...
In **3D mode**, this uses a real 3D group correction when the group grows beyond one label: only the label(s) you actually asked to correct get the full independent whole-cell walk described above — a neighbor that got folded in purely to keep its territory safe is **not** independently re-corrected across its own entire 3D extent (which could be a large, unrelated cell spanning much of the fish); it only takes part where it actually touches the target, via the same joint per-slice re-derivation [Correct Adjacent Labels](#correct-adjacent-labels) uses standalone. This keeps the cost of correcting one cell proportional to that cell's own size, not whatever else happens to be touching it. A final debris-cleanup pass (same golden-ratio floor as everywhere else) runs once at the end, scoped to just this group's own labels.
