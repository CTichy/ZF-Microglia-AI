...
- **3D (whole cell)** — corrects the entire cell. **Not a bounding box at any point.** This loops over 2D areas, one Z slice at a time, each with its OWN local neighborhood — the label's own CURRENT footprint on that one slice, padded — reusing the same already-correct 2D tools directly: no other label present in that local area → plain single-label 2D correction (exactly [Correct Label](#correct-label)'s own 2D mode); another label present → the same joint watershed split [Correct Adjacent Labels](#correct-adjacent-labels) uses, scoped to this label alone, so the local boundary against a genuinely adjacent cell gets properly re-derived right there too, not just excluded as an immovable wall. The label's own original Z range is corrected this way outright (it already exists on every one of those slices) — but first, for each consecutive pair of slices in that range, whichever one has the SMALLER bounding-box area inherits the bigger one's shape (foreign-protected paste) before either is corrected. Deliberately bounding-box area, not voxel count: a sparse, spread-out shape (a thin branch reaching wide, say) can have FEWER actual voxels than a small dense blob while still needing a much bigger local working window, since the window is sized from the shape's own min/max extent, not how many of its pixels happen to be filled in — comparing raw pixel counts would let the dense blob wrongly "win" and hand the spread-out slice a window sized for the wrong shape entirely. This guards against a single spuriously undersized original label on one slice — ordinary Cellpose-SAM prediction noise, not necessarily a real taper — sizing that slice's own local working window too tightly and clipping real signal a properly-sized window would have caught. It only ever widens the WINDOW the correction looks within; what actually gets painted still comes purely from the real signal threshold there, so a genuinely tapering cell still comes out correctly smaller slice by slice — only the risk of an undersized window is removed, not the correction's own honesty. **Slices are then corrected core-first, contact-last, not in plain Z order**: a quick lookahead finds which slices are already near or touching another label (a real neighbor, or a Protect Skin as Label sentinel) before anything is corrected; every other slice — the safely-isolated interior of the cell — is corrected first, and the genuinely contested slices are corrected last, in order of increasing proximity to that contact. This gives the contested boundary a fully-settled interior to anchor against once it's finally resolved, instead of settling into an arbitrary, order-dependent shape before the rest of the cell is even known — a real risk with a fixed min→max walk, where the very first slice corrected could be one sitting right against skin. Growth beyond the original range works the same way but one-directional: copying the current slice's own just-corrected shape onto the next Z first (foreign-protected, same as [Copy Label to Adjacent Slice](#copy-label-to-adjacent-slice)), then running the same local correction there — the moment that finds nothing to support the copied position, the copy is undone and growth in that direction stops, the true edge found. A hard cap (the same value as **Bbox padding** unless auto-grow overrides it) still bounds how far this can ever extend regardless, so a real, different, genuinely-touching structure can't pull growth along indefinitely just because it happens to overlap slice-to-slice.
...
**Label A** / **Label B** — type directly, or click each label in the viewer and use its own **Use selected** button. Order matters: A is the label the working rectangle is sized from.
