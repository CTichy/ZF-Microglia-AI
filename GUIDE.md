...
1. **Calibrates the contrast threshold** that best reproduces the labels the run just produced, straight from the raw signal — the exact same self-referential engine as [Calibrate Correct-Label Contrast (Section 9i)](#9i-calibrate-correct-label-contrast-from-cellpose-sam), just run against this run's own output instead of a hand-picked sample set. Call this calibrated value `lo`.
2. **Protects skin as its own label**, before any real cell is touched — the same engine as **Protect Skin as Label** (below), using the brain mask layer only to *seed* skin (never to clamp its result — see that section's own note on this), at the same `lo` real cells get in step 5, at its own dedicated, wider starting padding (25px — skin's own bounding box already spans nearly the whole frame on most slices, so it gets a bigger window from the start rather than sharing a real cell's tighter 15px default). Skin becomes a real, ordinary label (ID **-1**) at this point, with the same auto-grow/until-stable/joint-adjacent-resolution behavior Protect Skin as Label itself has — it adapts to whatever the cells currently look like (still raw, not yet auto-corrected) without ever modifying them.
3. **Removes debris skin just absorbed**, right here, before any real cell's own turn — same golden-ratio floor as every other debris pass in this plugin. Skin protection no longer clamps its own result against the brain mask (see below), so this is what actually catches any small stray blob it picked up along the way, by size alone.
4. **Figures out which real cells skin's own protection actually resolved a shared boundary against** (read from that step's own report, not a fresh scan — see below for why), then **resorts every real cell by Centroid Z** and partitions all of them into parallel-safe **waves** — the same partitioning either correction mode in step 5 uses, since both share the identical worst-case reach.
...
**Why step 4 reads skin's own report instead of scanning for touching pixels directly**: every joint watershed split in this plugin (this one included) deliberately clears a 1-voxel gap right at the freshly computed boundary between the two labels (so they don't end up literally touching again) — a fresh pixel-adjacency scan of skin's *final* territory would almost never find a real neighbor, even right after one was genuinely, jointly resolved against it. Skin's own protection step already tracks exactly which real labels it discovered as genuinely touching *before* that gap was cleared, so step 4 reuses that instead of re-deriving something the correction itself just erased the evidence for.
...
Click **Auto-correct Labels**. On success, the active Labels layer is updated in place (skin protected as its own label, every real cell corrected), the signal layer's own contrast lower limit moves to the calibrated threshold (upper limit left untouched), Protect Skin as Label's own section is populated as if that button had been clicked directly, and the full consolidated report (same format the chained Cellpose-SAM pipeline produces) appears in the log box below the status line.
