...
**In 3D mode**, this is simpler: since [Correct Label's own 3D mode](#correct-label) already resolves any genuinely adjacent label on its own, per slice, as part of its normal walk, auto-grow here is just a retry loop — widen the padding and re-run the whole-cell correction while it keeps touching the edge of its own local working area. The report still lists every OTHER label the final correction ended up near, for visibility, but there's no separate group-discovery step needed the way 2D has one. A final debris-cleanup pass (same golden-ratio floor as everywhere else) runs once at the end.
