...
7. **Softens every cell's contour (sanding)** — a further stage chained on afterward, gated by its own checkbox (Common Settings, above — see [Soften label contours (sanding)](#soften-label-contours-sanding-after-any-label-correction)), on by default. Purely geometric, runs on the full set of auto-corrected labels (skin excluded, same as every other tool that enumerates real cells only).
...
- **Signal layer** — the Image layer whose contrast window drives every intensity-threshold correction (Correct Label, Correct Adjacent Labels, the contrast-calibration sweep).
- **Labels layer** — the Labels layer every tool below reads from and writes back to.
- **Brain mask layer** — the Labels layer Protect Skin as Label and Auto-correct Existing Labels use to know what "outside the brain" means.
...
Shared by every tool below that needs a contrast-low threshold to decide what counts as real signal — **Protect Skin as Label** and **Auto-correct Existing Labels** today, and any future tool that needs the same calibration.
...
| Control | Recommended | What it does |
|---------|-------------|--------------|
| Auto-sweep for best contrast low (best_lo) | Checked | Read by Protect Skin as Label and Auto-correct Existing Labels — sweeps for best_lo against the active labels (same sweep as Calibrate Correct-Label Contrast, Tab 6); unchecked uses the Signal layer's own current contrast low instead, no sweep |
...
| Auto-correct Existing Labels | — | Runs the full Cellpose-SAM auto-correct pipeline (contrast sweep per 7c, skin protection, per-cell 2D-vs-skin/3D correction, debris cleanup) against the selected Labels layer as it stands — for a labels layer segmented earlier, any route or session. Reuses skin if already protected, refuses to correct cells without skin, decides "touching skin" by geometry (≤ 3 px), and lists possible non-microglia blobs (report only) |
