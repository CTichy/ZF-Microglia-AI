...
**Hide skin label in the viewer** — a checkbox right below Remove Skin Label, using the same field. Purely visual: makes that label ID fully transparent in the active Labels layer's own display, without touching any data at all — unlike Remove Skin Label, nothing is deleted, so toggling the checkbox back off restores it exactly, colors included. Useful whenever skin's own territory is visually cluttering the view of the real cells you're correcting. One caveat: a brand-new label created or painted while skin is hidden may briefly show a generic gray instead of its usual color, until you toggle this off and back on (which recomputes every label's color from scratch) — cosmetic only, never affects the label's actual ID or data.
