...
Given a 3D confocal volume (TIF or IMS), the plugin provides six tabs:
...
- **Tab 2 — Create MG Labels:** two ways to detect and label individual microglia in 3D. **Cellpose-SAM Segmentation** (`do_3D` inference → 3-component GMM cleanup → Krendl safe-merge → large-contact merge) is the recommended path — a fine-tuned foundation model that handles branching/overlapping cells far better than classical thresholding. The **Pixel Classifier** (Gaussian smooth → threshold → overlap-based union-find 3D stitching → volume filter) is a lighter, older-technology fallback for machines with no GPU at all. The tab automatically shows whichever one matches your active layer's background-removal mode (see below) — no manual switching needed. Only creation tools live here, plus two on/off checkboxes that chain sanding/auto-correction onto a fresh Cellpose-SAM run.
- **Tab 3 — Edit MG Labels:** every edit/correct/modify tool for an existing Labels layer (Resort, Remove Debris, Split, Join, Correct Label, Copy to Adjacent Slice, Correct Adjacent Labels, Protect Skin as Label, Auto-correct Labels, Save Labels), all reading their layers and label ID(s) from **one shared selector** at the top of the tab instead of each tool having its own copy.
- **Tab 4 — Statistics:** computes up to 51 morphological, spatial, and intensity features per labelled cell and exports a CSV. Always visible — shows an explanatory hint in place of the controls until at least one Labels layer exists, the same "explain instead of hide" pattern as Tab 2's method sections.
- **Tab 5 — AI Tools:** ground-truth polygon annotation plus launchers for MONAI and Cellpose-SAM training (dataset prep/crop extraction, hours-to-days training runs that survive napari closing). Always available — shows a disclaimer banner instead of hiding if your GPU is missing or under the recommended 8GB — see below.
- **Tab 6 — Sweeps & Utilities:** every GT-verification sweep tool (one per pipeline stage: MONAI segmentation, Pixel Classifier smoothing, Pixel Classifier labelling, Cellpose-SAM labelling, Cellpose-SAM checkpoint selection) plus two adjacent GT utilities, consolidated in one place instead of scattered across Tabs 1-5. Each tool still operates on its own tab's parameters and auto-applies its findings back there — this tab doesn't introduce a separate workflow, it just keeps the day-to-day tabs focused on running the pipeline rather than tuning it.
...
### Training scripts (Tab 5 — AI Tools only)

Tab 5 launches three project-specific research scripts (`prepare_data.py`, `train.py`, `train_xzyz.py`) as separate subprocesses. They ship with the plugin — bundled under `napari_zf_microglia_ai/training_scripts/` and installed as package data — so they're present for every install method (plain `git+https://` install or editable). If you want to point at a locally modified copy instead, the config file (`~/.config/napari-zf-microglia-ai/config.json`) has `monai_prepare_script_path`/`monai_train_script_path`/`cellpose_train_script_path` keys to override.
...
Only creation tools live here — every edit/correct/modify tool (Resort Labels, Remove Debris, Split, Join, Correct Label, Copy to Adjacent Slice, Correct Adjacent Labels, Protect Skin as Label, Auto-correct Labels, Save Labels) moved to **Tab 3 — Edit MG Labels**, below. **Tab 4 — Statistics** stays visible regardless of any of this, showing an explanatory hint instead of its controls until at least one Labels layer exists in the viewer.
...
**Auto-correct labels via contrast sweep after segmentation** (checkbox, on by default) — after a run finishes: calibrates the contrast threshold using the same **Auto-sweep**/**Reduce best_lo by** controls as Tab 3 — Edit MG Labels' own Contrast low (best_lo) calibration section (physically in Tab 3, read directly regardless of which tab is visible), then protects skin as its own label — always ID **-1** — at that threshold (or **reuses it if the labels already carry skin** — it never re-protects, and it stops with an error rather than correct any cell without skin) (Protect Skin as Label, using the **Brain mask layer** field below the checkbox only to *seed* skin, never to clamp its result — auto-filled from `<stem>_brain_mask`, since Tab 1 already creates and loads it alongside every `_ExtRm`/`_NoBG`/`_RndFill` layer — with its own wider 25px starting pad and the same per-slice auto-grow/until-stable and joint-adjacent-resolution behavior Protect Skin as Label itself has, adapting to whatever the still-raw cells currently look like without ever modifying them), **removes debris skin just absorbed** right there (no clamp means a small stray blob can get picked up — this sweeps it by size alone before any real cell's own turn), then resorts every cell by Centroid Z and figures out which cells **touch skin**, measured from skin's final geometry (a cell counts if, on any slice, it is within **3 px** of skin — the earlier approach, reading which cells had ever been folded into a skin-growth attempt, flagged 33 of 33 cells on a real fish although none was within 45 px of skin). Each cell is then corrected at that threshold, mode chosen by whether it touches skin: a **touching** cell is corrected in **2D, slice by slice, jointly against skin** (skin's own per-slice engine, writing back the cell's own side this time — skin can only ever be corrected in 2D, so a cell meeting it has to do so on skin's own terms); every other cell gets the same auto-grow + until-stable **3D** engine as Correct Label (fixed for this pipeline: pad 15px, growth 5px up to 10 attempts/slice, until-stable up to 100 passes/slice) — both modes batched into the same parallel-safe waves (up to 75% of CPU cores) instead of strictly one cell at a time, since two cells whose maximum-possible working areas can never overlap can't interact and are corrected concurrently regardless of mode; confirmed real speedup even on densely-packed fish, and confirmed byte-identical results to a fully sequential run for the 3D-corrected cells. Then runs Remove Debris once more over the whole result and screens for **possible non-microglia blobs** (≥ 40% of a cell's volume outside the brain mask, or of its surface within 3 px of skin — macrophages or leftover skin; **report only**, nothing is removed). Calibration and Centroid-Z resorting count microglia only (skin is ignored), and the CPU budget is shared between cells and slices (an earlier 6×6 nesting plus a full-volume copy per slice exhausted 119 GB of RAM). Produces one consolidated report covering every cell (not one report per cell). Required brain mask field means the run refuses to start if it's missing, checked before the segmentation itself begins. On completion, the signal layer's own contrast lower limit moves to the calibrated threshold (upper limit left exactly as it already was) and Tab 3's Protect Skin as Label section (touching/nearby-labels report, status) is filled in as if that button had been clicked directly. If **Soften label contours (sanding)** (Tab 2 Common Settings) is also on, a further stage chains onto this one afterward: every corrected cell first has any interior cavity/vesicle filled in (if that checkbox is also on), then its contour is softened — saved as `<stem>_cp_krendl_ac_snd.tif` on top of this stage's own `<stem>_cp_krendl_ac.tif`.

**Verify Cellprob / Large-contact (GT Sweep)** and **Build GT-Correction Package** — both moved to **Tab 6 — Sweeps & Utilities**.

**Re-run This Cell Only** — fixes one label without redoing the whole fish: crops to that label's own padded bounding box, re-runs `do_3D` + the same GMM/Krendl/large-contact/final-min-size cleanup on just the crop, then splices the result back in place of the old label. Only crop pieces that overlap the original label survive the splice. Works on the selected labels layer (any name), or finds `<image>_cellpose_labels` / `<image>_labels` (or a single same-shape labels layer) for the selected volume; asks instead of guessing if several qualify.

---

## Tab 3 — Edit MG Labels

Every edit/correct/modify tool for an existing Labels layer, gathered here instead of scattered through Tab 2. **One shared selector at the top of the tab** — Signal layer / Labels layer / Brain mask layer, plus **Label A** (single-label tools) and **Label B** (the two tools that need a second label) — drives every tool below; pick your layers and label(s) once, not per tool. Skin operations (Protect/Remove/Hide Skin Label) always act on the fixed sentinel ID **-1**, never on Label A/B. **Signal layer must be the raw, un-processed channel** (never `_ExtRm`/`_NoBG`) — it doubles as the real-signal source for skin protection, and a background-processed layer has already been zeroed everywhere outside the brain. All three layer combos **auto-populate together** the moment a matching `_brain_only_*` layer exists (e.g. right after Tab 1 runs) — Signal to the raw channel, Brain mask to `<stem>_brain_mask`, Labels to whatever was already created from it — without ever overriding a choice you've made yourself.

- **Sanding sigma XY / Z** (default 0.7/0.7 vox) — the sigma sliders for the "Soften label contours (sanding)" checkbox (Tab 2 Common Settings) live here, not there; that checkbox stays the only sanding control in Tab 2.
- **Fill interior cavities (vesicles) before sanding** (Tab 2 Common Settings, default on) — runs right before the blur: a real intracellular vesicle/vacuole shows up dark in the fluorescence channel, so a plain intensity threshold correctly excludes it as "not signal" even though it's genuine cell material, leaving a literal 3D-enclosed cavity. This fills it back in, fully in 3D (not per Z/Y/X slice, not a convex hull — a cell's own concave/branching shape is left exactly as it is; only background voxels fully enclosed by the label are ever touched). Cells only, never the skin sentinel — skin's own "cavities" are real cells sitting inside the brain, not vesicles.
- **Contrast low (best_lo) calibration** — **Auto-sweep for best contrast low (best_lo)** (default on) + **Reduce best_lo by** (default 3, range 0-6): the calibration Protect Skin as Label and Auto-correct Labels both use. With Auto-sweep on, sweeps for best_lo then subtracts the reduction; with it off, uses the Signal layer's own current contrast low directly (no sweep). The sweep itself — how many cells/slices, edge margin, sweep steps — is the exact same sweep **Calibrate Correct-Label Contrast** (Tab 6) runs; tune those sample-selection parameters there, they're shared.
- **Resort Labels** — renumber 1…N by size, centroid Z/Y/X
- **Remove Debris** — sweeps the selected layer for anything below the golden-ratio-relaxed volume floor, per connected fragment (not per label ID); sweeps skin (-1) too, whenever present
- **Split Label** — watershed split of Label A into N parts
- **Join Labels** — the inverse of Split: merges Label B into Label A
- **Correct Label** — regenerates Label A's shape (2D current slice, or 3D whole cell) from the shared Signal layer's own live contrast window (signal = intensity at/above the lower contrast limit); Label A may be any positive ID or **-1** for skin, though **3D is disabled for skin specifically** (a 3D correction walks the WHOLE outside-brain territory slice by slice — real computation cost for no benefit over Protect Skin as Label's own dedicated, far cheaper trim; 2D skin correction still works normally) — 3D walks the label's own Z range mid-outward (not plain Z order), topping up each slice's watershed seed with whichever already-corrected or original neighboring slice(s) are in range before correcting it, and jointly resolves against a real neighboring label wherever one's actually nearby — optional **auto-grow**: retries with a bigger padded area if the result touches its own edge (catching signal a too-small pad would cut off), auto-folding in any neighboring label the growing area reaches instead of encroaching on it, capped at a configurable max number of retries; optional **until-stable** (3D): keeps re-running each slice, reseeded from its own previous result, until it stops changing
- **Copy Label to Adjacent Slice** — copies Label A's shape from the current slice onto the next/previous slice, e.g. to patch a broken/missing cross-section
- **Correct Adjacent Labels** — jointly regenerates Label A and Label B on the current slice (working area scoped to Label A alone), cut placed by watershed seeded at each label's own existing footprint — same auto-grow option as Correct Label, seeded with both labels from the start, plus its own optional **until-stable**: keeps re-running until every label in the group matches its previous pass
- **Protect Skin as Label** — always label **-1**, never Label A/B. Gets `lo` from the shared Contrast low (best_lo) calibration section above, then fills everything outside the shared Brain mask layer with skin and trims it to real signal, **slice by slice independently by calling Correct Label's own single-slice engine directly**, once per slice — not a separate approximation of it, the exact same function the button itself calls, so an automated trim now matches selecting skin by hand on a slice and clicking Correct Label byte-for-byte; not a cross-slice 3D whole-cell walk — skin's candidate territory is already fully known per slice from the brain mask, and spans nearly the whole stack, so walking that whole-object machinery per slice would be pure wasted computation here. Each slice still gets its own **per-slice auto-grow + until-stable**, starting from its own wider 25px default pad (skin's own bounding box already spans nearly the whole frame on most slices, so it gets a bigger window than a real cell's own 15px default from the start; on by default: growth 5px up to 10 attempts/slice, until-stable up to 100 passes/slice, including dynamically folding in a newly-touched neighbor mid-growth), run **in parallel across up to 75% of CPU cores** (every slice is fully independent), and wherever a real cell is present, the boundary is **jointly resolved** against it via the same watershed Correct Adjacent Labels uses (not just excluded) — but only skin's own resulting territory is ever written back, the touching cell's label is never modified (a cell's own existing pixels are always its own watershed marker, which structurally can't be reassigned). **Does NOT clamp against the brain mask** — a **Remove Debris** pass runs automatically right after protection (by hand here, and automatically in the Cellpose-SAM auto-correct pipeline) to sweep up whatever small stray blob skin absorbed, by size alone — reports which real labels end up touching/nearby skin, worth a closer look; a companion **Remove Skin Label** button clears -1 again whenever it's no longer needed, and a **Hide skin label in the viewer** checkbox makes -1 fully transparent in the display only (no data change, every other label keeps its own real distinct color both while hidden and after toggling back off) whenever it's visually cluttering a correction session. **Refuses to protect twice** — if the layer already carries the skin label it says so and does nothing (click Remove Skin Label first to start over)
- **Auto-correct Labels** — runs the exact same 6-step auto-correct pipeline the Cellpose-SAM Segmentation section's own checkbox chains onto a fresh run (contrast sweep per the shared calibration controls above, skin protection + debris cleanup, per-cell correction — 2D against skin for a touching cell, 3D otherwise — then a final debris cleanup), but against the selected Labels layer as it currently stands instead of a just-produced one, so a labels layer segmented earlier (any route, any prior session, or one loaded via Load Labels layer) can get the same treatment without re-running Cellpose-SAM. Uses the shared Signal/Labels/Brain mask layer selector, the shared calibration controls, and Protect Skin as Label's own Bbox padding field — no duplicate set of fields. If skin is already protected (e.g. you clicked Protect Skin as Label first) it is reused as is; if skin cannot be protected the run stops before touching any cell. The report ends with the report-only list of possible non-microglia blobs. **If sanding is on, chains the same fill-cavities + soften-contours stage afterward** as the Cellpose-SAM route does (a real gap this now closes — it never used to) — this button still never saves a file on its own, before or after
- **Save Labels** — explicit file dialog (edit labels in napari before saving)

> **Soften label contours (sanding)** itself (the on/off checkbox) stays in Tab 2's Common Settings — shared by Correct Label, Correct Adjacent Labels, Auto-correct Labels, and Cellpose-SAM's own auto-correct stage: after any of these, the touched label(s) first have any interior cavity/vesicle filled in (if that checkbox is also on), then get their contours smoothed using this tab's Sigma XY/Z sliders (foreign-protected, purely geometric — rounds off blocky voxel edges without reshaping the cell).

---

## Tab 4 — Statistics

Computes up to 51 features per labelled cell and exports a CSV. For what each column means, see [GUIDE.md §12](GUIDE.md#12-statistics-csv--all-columns-explained); for the algorithm/formula behind each one, see [STATISTICS_GUIDE.md](STATISTICS_GUIDE.md).
...
**Score Against GT** — moved to **Tab 6 — Sweeps & Utilities**; scores any two Labels layers already in the viewer against each other.
...
Any other SMTP-over-SSL (implicit TLS, not STARTTLS) provider on a fixed port works the same way, just with a different server/port — see **GUIDE.md Section 13a** for the full walkthrough, more provider examples, and a note on why STARTTLS-only providers (e.g. Office 365 on port 587) aren't currently supported.

A switch below that selects one of two mutually-exclusive groups:

### MONAI Training

- **GT Annotation** — hand-draw polygon boundaries on key slices (every ~10 slices) of a chosen Image layer, interpolate along Z (point-to-point, with propagation past a reference slice), then rasterize to `brain_mask`/`skin_mask`/`brain_only`/`skin_only` TIFFs, saved next to the source file (same `<parent>/<stem>/` convention as Tabs 1-4).
...
**Email notification** — unlike the log-tail/patience/recommended-checkpoint features above, this one doesn't depend on the GUI polling loop at all, and so doesn't require ever reopening napari. When that launcher's "Email me when this training run stops" checkbox is ticked (and a recipient is configured in Tab 6), the launched process is actually a small self-contained supervisor script (stdlib-only: `subprocess`/`smtplib`/`re`) that runs the real training command, waits for it to exit, parses the same log for the best checkpoint, and emails a completion report — *then* exits. Since the supervisor itself is what's detached, the email fires on its own schedule regardless of whether napari is running at that moment. Clicking "Stop Training" kills the whole process tree (supervisor included) before it reaches the email step, so a manual stop correctly sends no notification — only unattended completions/crashes do.
...
**Drift View in 3D** — General category, nothing to do with GT. Start/Stop Drift button + a Speed slider: slowly, continuously tumbles the 3D camera around whatever's currently in view (switches to 3D display automatically), like drifting past it in space. Purely a camera effect (`viewer.camera.angles`) — never touches any layer's own data. Rotates all three axes (pitch/roll/yaw) freely and continuously, never stalling on one — a real bug fixed by composing a proper 3D rotation (`scipy`'s `Rotation`) each tick instead of incrementing napari's three Euler-angle numbers independently, which hit napari's own `'xyz'`-decomposition's ±90° limit on its middle axis and visibly stuck there.

**Autosave Labels Layer** — General category. Checkbox (default on) + an interval spinbox/unit dropdown (minutes or hours, default **30 minutes**): while checked, periodically saves ONLY the Labels layer currently picked in Tab 3's own shared layer selector to `<output>/<name>_recovery.tif`, overwriting the previous save each time — a crash loses at most one interval's worth of manual editing. Scoped to that one layer specifically (not every Labels layer in the viewer, like an earlier version did) — a `brain_mask` layer is cheap to regenerate via Tab 1 and was never what a crash actually cost you.

**Build GT-Correction Package** — packages the most-advanced Cellpose-SAM correction stage available (sanded > auto-corrected > Krendl-only, auto-picked when the source image is browsed) into a folder + zip (source image + `cp_corrected.tif` + optional raw pre-merge masks + optional `brain_mask.tif` — needed on the reviewer's end to use Protect Skin as Label — + a lightweight per-cell CSV + `GROUND_TRUTH_CREATION_GUIDE.md`), matching the exact layout this project has hand-assembled for every fish sent out for manual GT correction. The corrected result becomes future training data via Tab 5's Extract XZYZ Patches. On-disk naming for every stage is cumulative: `cp.tif` (raw) → `cp_krendl.tif` (+Krendl) → `cp_krendl_ac.tif` (+auto-correct) → `cp_krendl_ac_snd.tif` (+sanding).

> **Calibrate branch_radius (from GT)** stayed in Tab 5 rather than moving here — it's a direct input to the Train Cellpose-SAM parameters right below it, not a standalone verification tool.
...
| `conda env create` fails on Windows with `Didn't find wheel for cucim-cu12` | Fixed as of this `environment.yml` — `cucim-cu12` (Linux/WSL2-only, no Windows wheels) is now Linux-only. `git pull` if you're on an older clone; only Tab 4's GPU-batch stats path needs it, and it falls back to CPU cleanly without it |
...
| Tab 5 (AI Tools) shows a red/amber banner | Expected, not an error — the tab is always available regardless of GPU; the banner just sets expectations (CPU fallback, or try a lower `batch_size`) — see Tab 5 section above |
| Tab 5's "Launch Training" buttons error with script-not-found | The plugin's guessed script path is wrong for your layout — override it in `~/.config/napari-zf-microglia-ai/config.json` (see "Training scripts" above) |
