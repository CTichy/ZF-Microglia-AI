...
### Pixel Classifier — 3D Connected-Component Labels

Fully self-contained: 3D Gaussian smooth → re-threshold → per-slice hole-fill → true 3D connected components (26-connectivity) → volume filter → sequential renumber. Dispatches to CUDA, Apple MPS (threaded CPU), or plain threaded CPU automatically. Best on `_NoBG` layers (background removed everywhere, not just outside the brain).
...
- **Correction Settings (Bbox / Auto-grow / Stability)** — Bbox padding (default 15px), Auto-grow (default on) + Growth step (default 5px) + Max growth iterations (default 20), and Keep re-running until the shape stabilizes (default on) + Max stability passes (default 50): one shared set of controls for **Correct Label**, **Correct Adjacent Labels**, **Protect Skin as Label**, and **Auto-correct Labels** (standalone or chained onto Cellpose-SAM Segmentation) — changing a value here changes it for all four at once, read live at the moment each one's own computation actually starts.
...
- **Correct Adjacent Labels** — jointly regenerates Label A and Label B on the current slice (working area scoped to Label A alone), cut placed by watershed seeded at each label's own existing footprint — same shared Correction Settings as Correct Label, seeded with both labels from the start, plus Keep-until-stable: keeps re-running until every label in the group matches its previous pass
- **Protect Skin as Label** — always label **-1**, never Label A/B. Gets `lo` from the shared Contrast low (best_lo) calibration section above, then fills everything outside the shared Brain mask layer with skin and trims it to real signal, **slice by slice independently by calling Correct Label's own single-slice engine directly**, once per slice — the exact same function the button itself calls, byte-for-byte identical to selecting skin by hand on a slice and clicking Correct Label; not a cross-slice 3D whole-cell walk — skin's candidate territory is already fully known per slice from the brain mask, and spans nearly the whole stack, so walking that whole-object machinery per slice would be pure wasted computation here. Each slice still gets its own **per-slice auto-grow + until-stable**, reading the same shared Correction Settings every other correction tool uses, run **in parallel across up to 75% of CPU cores** (every slice is fully independent), and wherever a real cell is present, the boundary is **jointly resolved** against it via the same watershed Correct Adjacent Labels uses (not just excluded) — but only skin's own resulting territory is ever written back, the touching cell's label is never modified (a cell's own existing pixels are always its own watershed marker, which structurally can't be reassigned). **Does NOT clamp against the brain mask** — a **Remove Debris** pass runs automatically right after protection (by hand here, and automatically in the Cellpose-SAM auto-correct pipeline) to sweep up whatever small stray blob skin absorbed, by size alone — reports which real labels end up touching/nearby skin, worth a closer look; a companion **Remove Skin Label** button clears -1 again whenever it's no longer needed, and a **Hide skin label in the viewer** checkbox makes -1 fully transparent in the display only (no data change, every other label keeps its own real distinct color both while hidden and after toggling back off) whenever it's visually cluttering a correction session. **Refuses to protect twice** — if the layer already carries the skin label it says so and does nothing (click Remove Skin Label first to start over)
- **Auto-correct Labels** — runs the exact same 6-step auto-correct pipeline the Cellpose-SAM Segmentation section's own checkbox chains onto a fresh run (contrast sweep per the shared calibration controls above, skin protection + debris cleanup, per-cell correction — 2D against skin for a touching cell, 3D otherwise — then a final debris cleanup), but against the selected Labels layer as it currently stands instead of a just-produced one, so a labels layer segmented earlier (any route, any session, or one loaded via Load Labels layer) can get the same treatment without re-running Cellpose-SAM. Uses the shared Signal/Labels/Brain mask layer selector, the shared calibration controls, and the shared Correction Settings — for every real cell's own correction, and for skin's own trim too. If skin is already protected (e.g. you clicked Protect Skin as Label first) it is reused as is; if skin cannot be protected the run stops before touching any cell. The report ends with the report-only list of possible non-microglia blobs. **If sanding is on, chains the same fill-cavities + soften-contours stage afterward** as the Cellpose-SAM route does — this button still never saves a file on its own, before or after
...
Always visible, regardless of GPU (checked once at plugin startup). A banner at the top adjusts to your GPU situation instead of hiding the tab: red/bold with no CUDA GPU ("CPU fallback — days to months for a full training run instead of hours"), amber/bold with a GPU under the recommended 8GB ("may still work with a reduced `batch_size`"), or a quiet green confirmation once the recommendation is met. GT Annotation needs no GPU at all.
...
- **Extract XZYZ Patches** — generates 2D crops in all three orientations (XY native, XZ/YZ Z-stretched to match XY's pixel scale) from a full-fish image + GT labels pair. **Cleans truncated incidental-neighbor labels by default**: a crop framed around one cell can graze the corner of a different nearby cell, keeping only a tiny sliver as a valid-looking (but wildly wrong-centered) training label — any label below a configurable visible-fraction threshold (default 90%) of its true full-slice size gets zeroed out automatically right after generation, backing up the crop folder first.
...
| `conda env create` fails on Windows with `Didn't find wheel for cucim-cu12` | `cucim-cu12` (Linux/WSL2-only, no Windows wheels) is Linux-only in `environment.yml`; only Tab 4's GPU-batch stats path needs it, and it falls back to CPU cleanly without it |
