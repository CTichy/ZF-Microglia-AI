...
- **Correct Label** — regenerates a label's shape (2D current slice, or 3D whole cell) from the signal layer's own live contrast window (signal = intensity at/above the lower contrast limit) — 3D loops over 2D local areas slice by slice rather than one bounding box, jointly resolving against a real neighboring label wherever one's actually nearby — optional **auto-grow**: retries with a bigger padded area if the result touches its own edge (catching signal a too-small pad would cut off), auto-folding in any neighboring label the growing area reaches instead of encroaching on it, capped at a configurable max number of retries
...
- **Correct Adjacent Labels** — jointly regenerates two touching labels on the current slice (working area scoped to Label A alone), cut placed by watershed seeded at each label's own existing footprint — same auto-grow option as Correct Label, seeded with both labels from the start
