...
**Auto-correct labels via contrast sweep after segmentation** (checkbox, on by default) — after a run finishes: calibrates the contrast threshold that best reproduces the fresh labels from the raw signal, protects skin as its own label at that SAME threshold (Protect Skin as Label, using the **Brain mask layer** field below the checkbox only to *seed* skin, never to clamp its result — auto-filled from `<stem>_brain_mask`, since Tab 1 already creates and loads it alongside every `_ExtRm`/`_NoBG`/`_RndFill` layer — with the same per-slice auto-grow/until-stable and joint-adjacent-resolution behavior Protect Skin as Label itself has, adapting to whatever the still-raw cells currently look like without ever modifying them), **removes debris skin just absorbed** right there (no clamp means a small stray blob can get picked up — this sweeps it by size alone before any real cell's own turn), resorts every cell by Centroid Z, then corrects every cell at that threshold with the same auto-grow + until-stable 3D engine as Correct Label (fixed for this pipeline: pad 15px, growth 5px up to 10 attempts/slice, until-stable up to 100 passes/slice) — batched into parallel-safe waves (up to 75% of CPU cores) instead of strictly one cell at a time, since two cells whose maximum-possible working areas can never overlap can't interact and are corrected concurrently; confirmed real speedup even on densely-packed fish, and confirmed byte-identical results to a fully sequential run — and finally runs Remove Debris once more over the whole result. Produces one consolidated report covering every cell (not one report per cell). Required brain mask field means the run refuses to start if it's missing, checked before the segmentation itself begins. On completion, the signal layer's own contrast is set to the calibrated threshold and Protect Skin as Label's own section (skin-ID field, touching/nearby-labels report, status) is filled in as if that button had been clicked directly.
...
**Drift View in 3D** — General category, nothing to do with GT. Start/Stop Drift button + a Speed slider: slowly, continuously tumbles the 3D camera around whatever's currently in view (switches to 3D display automatically), like drifting past it in space. Purely a camera effect (`viewer.camera.angles`) — never touches any layer's own data.
