...
- **Contrast low (best_lo) calibration** — **Auto-sweep for best contrast low (best_lo)** (default on) + **Reduce best_lo by** (default 3, range 0-6): the calibration Protect Skin as Label and Auto-correct Existing Labels both use. With Auto-sweep on, sweeps for best_lo then subtracts the reduction; with it off, uses the Signal layer's own current contrast low directly (no sweep). The sweep itself — how many cells/slices, edge margin, sweep steps — is the exact same sweep **Calibrate Correct-Label Contrast** (Tab 6) runs; tune those sample-selection parameters there, they're shared.
...
- **Auto-correct Existing Labels** — runs the exact same 6-step auto-correct pipeline the Cellpose-SAM Segmentation section's own checkbox chains onto a fresh run (contrast sweep per the shared calibration controls above, skin protection + debris cleanup, per-cell correction — 2D against skin for a touching cell, 3D otherwise — then a final debris cleanup), but against the selected Labels layer as it currently stands instead of a just-produced one, so a labels layer segmented earlier (any route, any prior session, or one loaded via Load Labels layer) can get the same treatment without re-running Cellpose-SAM. Uses the shared Signal/Labels/Brain mask layer selector, the shared calibration controls, and Protect Skin as Label's own Bbox padding field — no duplicate set of fields. If skin is already protected (e.g. you clicked Protect Skin as Label first) it is reused as is; if skin cannot be protected the run stops before touching any cell. The report ends with the report-only list of possible non-microglia blobs
- **Save Labels** — explicit file dialog (edit labels in napari before saving)

> **Soften label contours (sanding)** itself (the on/off checkbox) stays in Tab 2's Common Settings — shared by Correct Label, Correct Adjacent Labels, and Cellpose-SAM's own auto-correct stage: after any of these, the touched label(s) get their contours smoothed using this tab's Sigma XY/Z sliders (foreign-protected, purely geometric — rounds off blocky voxel edges without reshaping the cell).
