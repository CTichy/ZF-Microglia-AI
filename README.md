...
- **Remove Debris** — sweeps the active layer for anything below the golden-ratio-relaxed volume floor, per connected fragment (not per label ID); sweeps Protect Skin as Label's own skin label too, whenever present
- **Split Label** — watershed split of a merged blob into N parts
- **Join Labels** — the inverse of Split: merges Label B into Label A
- **Correct Label** — regenerates a label's shape (2D current slice, or 3D whole cell) from the signal layer's own live contrast window (signal = intensity at/above the lower contrast limit); accepts any positive label ID or **-1** for skin, though **3D is disabled for skin specifically** (a 3D correction walks the WHOLE outside-brain territory slice by slice — real computation cost for no benefit over Protect Skin as Label's own dedicated, far cheaper trim; 2D skin correction still works normally) — 3D walks the label's own Z range mid-outward (not plain Z order), topping up each slice's watershed seed with whichever already-corrected or original neighboring slice(s) are in range before correcting it, and jointly resolves against a real neighboring label wherever one's actually nearby — optional **auto-grow**: retries with a bigger padded area if the result touches its own edge (catching signal a too-small pad would cut off), auto-folding in any neighboring label the growing area reaches instead of encroaching on it, capped at a configurable max number of retries; optional **until-stable** (3D): keeps re-running each slice, reseeded from its own previous result, until it stops changing
- **Copy Label to Adjacent Slice** — copies a label's shape from the current slice onto the next/previous slice, e.g. to patch a broken/missing cross-section; accepts any positive label ID or -1 for skin
- **Correct Adjacent Labels** — jointly regenerates two touching labels on the current slice (working area scoped to Label A alone), cut placed by watershed seeded at each label's own existing footprint; either label may be any positive ID or -1 for skin — same auto-grow option as Correct Label, seeded with both labels from the start, plus its own optional **until-stable**: keeps re-running until every label in the group matches its previous pass
- **Protect Skin as Label** — first **calibrates `lo` with the contrast sweep against the loaded labels** (the same sweep as Auto-correct's first step; it no longer silently uses the signal layer's current low limit, e.g. the IMS-load value — that layer's low limit is set to the result, high untouched), then fills everything outside the brain mask with a real, protected label (ID -1 by default) then trims it to real signal, **slice by slice independently by calling Correct Label's own single-slice engine directly**, once per slice — not a separate approximation of it, the exact same function the button itself calls, so an automated trim now matches selecting skin by hand on a slice and clicking Correct Label byte-for-byte (confirmed directly — an earlier version reimplemented its own growth loop instead, which subtly didn't always match); not a cross-slice 3D whole-cell walk — skin's candidate territory is already fully known per slice from the brain mask, and spans nearly the whole stack, so walking that whole-object machinery per slice would be pure wasted computation here. Each slice still gets its own **per-slice auto-grow + until-stable**, starting from its own wider 25px default pad (skin's own bounding box already spans nearly the whole frame on most slices, so it gets a bigger window than a real cell's own 15px default from the start; on by default: growth 5px up to 10 attempts/slice, until-stable up to 100 passes/slice, including dynamically folding in a newly-touched neighbor mid-growth), run **in parallel across up to 75% of CPU cores** (every slice is fully independent), and wherever a real cell is present, the boundary is **jointly resolved** against it via the same watershed Correct Adjacent Labels uses (not just excluded) — but only skin's own resulting territory is ever written back, the touching cell's label is never modified (a cell's own existing pixels are always its own watershed marker, which structurally can't be reassigned). **Does NOT clamp against the brain mask** — an earlier version did, to stop skin absorbing small debris/possible-cell blobs inside the brain, but that clamp also blocked legitimate correction along skin's real inner boundary that manual Correct Label never had to fight; the clamp is removed, and a **Remove Debris** pass now runs automatically right after protection (by hand here, and automatically in the Cellpose-SAM auto-correct pipeline) to sweep up whatever small stray blob skin absorbed, by size alone — reports which real labels end up touching/nearby skin, worth a closer look; a companion **Remove Skin Label** button clears it again whenever it's no longer needed, and a **Hide skin label in the viewer** checkbox makes it fully transparent in the display only (no data change, every other label keeps its own real distinct color both while hidden and after toggling back off) whenever it's visually cluttering a correction session — uses a custom colormap that forces napari's fast array-index texture cast rather than its default (numba typed-dict) one, since the default is markedly slower for a whole-volume re-cast in 3D display mode. **Refuses to protect twice** — if the layer already carries the skin label it says so and does nothing (click Remove Skin Label first to start over)
- **Auto-correct Existing Labels** — runs the exact same 6-step auto-correct pipeline the Cellpose-SAM Segmentation section's own checkbox chains onto a fresh run (contrast sweep, skin protection + debris cleanup, per-cell correction — 2D against skin for a touching cell, 3D otherwise — then a final debris cleanup), but against the active Labels layer as it currently stands instead of a just-produced one, so a labels layer segmented earlier (any route, any prior session, or one loaded via Load Labels layer) can get the same treatment without re-running Cellpose-SAM. Reuses Protect Skin as Label's own Signal layer / Brain mask layer / Bbox padding fields rather than a duplicate set right below it. If skin is already protected (e.g. you clicked Protect Skin as Label first) it is reused as is; if skin cannot be protected the run stops before touching any cell. The report ends with the report-only list of possible non-microglia blobs
